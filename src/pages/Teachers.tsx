...
const Teachers = () => {
	...
	const fetchTeachers = async () => {
		try {
			...
			console.log("apiUrl", apiUrl(`teachers.php?${params.toString()}`));
		...
		const periodMatches =
			selectedPeriod === "All"
				...
				: (() => {
						...
						return (
							(failedVal !== undefined && failedVal !== null) ||
	...
	const totalPages = Math.max(
		1,
		Math.ceil(sortedTeachers.length / itemsPerPage)
	);
	...
	const handleUpdateTeacher = async () => {
		...
		try {
			const response = await fetch(
				apiUrl(`teachers.php?id=${selectedTeacher.id}`),
				{
					method: "PUT",
					headers: {
						"Content-Type": "application/json",
			...
			if (response.ok && result.success) {
				const extra =
					result.filename && result.filesize
						? ` (saved as ${result.filename}, ${result.filesize} bytes)`
						: "";
				toast({
	...
	const downloadTemplate = () => {
		let csvContent =
			"FacultyNo,FacultyName,EnrolledStudents,P1_Failed,P1_Percent,P1_Category,P2_Failed,P2_Percent,P2_Category";
		let sampleData =
			"14-007-F,ADORMIE CORRALES MACARIO,184,18,9.78,GREEN (0.01%-10%),,,\n" +
		...
		<div className="space-y-6">
			...
			<Card>
				<CardHeader>
					...
					<CardDescription>Find teachers by name or ID</CardDescription>
				</CardHeader>
				<CardContent>
									...
									<Select
										...
										onValueChange={(value) =>
											setFilters((prev) => ({
												...prev,
												department:
													value === "__ALL_DEPARTMENTS__" ? "" : value,
											}))
										}
									>
										<SelectTrigger>
								...
								<div>
									...
									<Select
										...
										onValueChange={(value) =>
											setFilters((prev) => ({
												...prev,
												zone: value === "__ALL_ZONES__" ? "" : value,
											}))
										}
									>
										<SelectTrigger>
							...
							<TableHeader>
								<TableRow>
									{visibleColumns.teacher_id && (
										<TableHead className="w-40 whitespace-nowrap text-left">
											Faculty Number
										</TableHead>
									)}
									{visibleColumns.name && (
										<TableHead className="w-56 whitespace-nowrap text-left">
											Faculty Name
										</TableHead>
									)}
									{visibleColumns.department && (
										<TableHead className="w-44 whitespace-nowrap text-left">
											Department
										</TableHead>
									)}
									{visibleColumns.enrolled && (
										<TableHead className="w-64 whitespace-nowrap text-left">
									...
									{visibleColumns.zone && (
										<TableHead className="w-28 whitespace-nowrap text-left">
											Zone
										</TableHead>
									)}
									{visibleColumns.actions && (
										<TableHead className="w-32 whitespace-nowrap text-left">
											Actions
										</TableHead>
									)}
								</TableRow>
									...
									<TableRow key={teacher.id}>
										...
										{visibleColumns.p1_performance && (
											<>
												<TableCell className="w-44 whitespace-nowrap">
													<div className="text-sm">
														{Number(teacher.p1_failed) || 0}
													</div>
												</TableCell>
												<TableCell className="w-44 whitespace-nowrap">
													<div className="text-sm text-muted-foreground">
														{(() => {
															const pct = percentFromData(
																teacher.p1_percent,
																teacher.p1_failed,
																teacher.enrolled_students
															);
				...
				<DialogContent className="sm:max-w-[425px]">
					...
					<div className="grid gap-4 py-4">
						...
						<div className="grid grid-cols-4 items-center gap-4">
							...
							<Select
								value={formData.zone}
								onValueChange={(value: Teacher["zone"]) =>
			...
			<Dialog open={isEditDialogOpen} onOpenChange={setIsEditDialogOpen}>
				<DialogContent className="sm:max-w-[425px]">
					...
					<div className="grid gap-4 py-4">
						<div className="grid grid-cols-4 items-center gap-4">
							<Label htmlFor="edit_teacher_id" className="text-right">
								Teacher ID*
							</Label>
							<Input
								id="edit_teacher_id"
						...
						<div className="grid grid-cols-4 items-center gap-4">
							<Label htmlFor="edit_middle_name" className="text-right">
								Middle Name
							</Label>
							<Input
								id="edit_middle_name"
						...
						<div className="grid grid-cols-4 items-center gap-4">
							<Label htmlFor="edit_zone" className="text-right">
								Zone
							</Label>
							<Select
								value={formData.zone}
			...
			<Dialog open={isViewDialogOpen} onOpenChange={setIsViewDialogOpen}>
				<DialogContent className="sm:max-w-[425px]">
					<DialogHeader>
						...
						<DialogDescription>View teacher information</DialogDescription>
					</DialogHeader>
					{selectedTeacher && (
						<div className="grid gap-4 py-4">
							<div className="grid grid-cols-4 items-center gap-4">
							...
							<div className="grid grid-cols-4 items-center gap-4">
								...
								<div className="col-span-3">
									{selectedTeacher.position || "No position"}
								</div>
							</div>
							<div className="grid grid-cols-4 items-center gap-4">
							...
							<div className="grid grid-cols-4 items-center gap-4">
								...
								<div className="col-span-3">
									{selectedTeacher.notes || "No notes"}
								</div>
							</div>
							<div className="grid grid-cols-4 items-center gap-4">
